use bevy::{app::App, color::palettes::css::WHITE, ecs::system::Commands, math::I16Vec3, platform::collections::HashMap, prelude::*, window::PrimaryWindow, DefaultPlugins};

use crate::{chunk::{Block, BlockGrid, Chunk, ChunkGrid, CHUNK_SIZE_F32}, game::camera_movement::MovableCamera};
...
fn setup(
    ...
    for x in MIN_CHUNK..=MAX_CHUNK {
        for y in MIN_CHUNK..=MAX_CHUNK {
            for z in MIN_CHUNK..=MAX_CHUNK {
                if x.abs() % 2 == 1 || y.abs() % 2 == 1 || z.abs() % 2 == 1 { continue }
    ...
    for (chunk_position, mesh) in chunk_meshes {
        ...
        chunk_world.meshes.insert(chunk_position, mesh_handle.clone());
        commands.spawn((
            Mesh3d(mesh_handle),
...
fn update_debug_position_text(
    ...
    let int_camera_position = IVec3::new(camera_position.x as i32, camera_position.y as i32, camera_position.z as i32);
